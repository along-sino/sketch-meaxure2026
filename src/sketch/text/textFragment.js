...
exports.getFragments = exports.getFragmentsCount = void 0;
function getFragmentsCount(layer) {
    let fragments = layer.sketchObject.attributedString().treeAsDictionary().value.attributes;
    return fragments.length;
}
exports.getFragmentsCount = getFragmentsCount;
function getFragments(layer) {
    try {
        let TextMSAttributedString = layer.sketchObject.attributedString();
        let fragments = TextMSAttributedString.treeAsDictionary().value.attributes;
        let results = [];
        ...
        if (styleBase.fontAxes) {
            ...
            styleBase.fontAxes = Object.assign({}, {
        ...
        for (let i = 0; i < fragments.length; i++) {
            ...
            results.push({
                ...
                text: fragment.text,
                style: Object.assign(Object.assign({}, styleBase), { textColor: textColor, fontSize: fontSize, fontFamily: fontFamily, fontWeight: fontWeight, textStrikethrough: fragment.NSStrikethrough ? 'single' : null, textUnderline: fragment.NSUnderline ? 'single' : null }),
        ...
        // Fallback for Sketch 2025.1 compatibility issues
        console.warn('Error getting text fragments, using fallback:', error);
        let textLayer = layer;
        return [{
                location: 0,
                length: textLayer.text.length,
                text: textLayer.text,
                style: textLayer.style,
                defaultLineHeight: getDefaultLineHeightForFont(textLayer.style.fontFamily, textLayer.style.fontSize),
            }];
    }
}
