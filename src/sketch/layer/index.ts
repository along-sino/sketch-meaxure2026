...
export function extendLayer() {
    ...
    Object.defineProperty(target, "CSSAttributes", {
        get: function () {
            // Sketch 2026+ 兼容处理：CSSAttributes 可能不存在
            if (typeof this.sketchObject.CSSAttributes !== 'function') {
                return [];
            }
            ...
            let css = [];
            let count = layerCSSAttributes.count ? layerCSSAttributes.count() : (layerCSSAttributes.length || 0);
